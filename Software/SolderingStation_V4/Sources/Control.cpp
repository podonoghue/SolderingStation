...
void Control::initialise() {
   ...
   ADConverter::setAveraging(AdcAveraging_8);
   ...
   static constexpr uint8_t ZERO_CROSSING_DAC_THRESHOLD = (CURRENT_LIMIT*OVERLOAD_VOLT_PER_AMP)*(ZeroCrossingComparator::MAXIMUM_DAC_VALUE/CMP_REF_VOLTAGE);
   ...
   analogueMultiplexor.setOutput(PinDriveStrength_Low, PinDriveMode_PushPull, PinSlewRate_Slow);

   // Enable amplifier input clamp
   clamp.setOutput(PinDriveStrength_Low, PinDriveMode_PushPull, PinSlewRate_Slow);
}
...
//      clamp.off();
   ...
   else if (adcChannel == identifyCh1.CHANNEL) {
      identifyCh2.startConversion(AdcInterrupt_Enabled);
      ch1.setIdValue(result);
      return;
   }
   else if (adcChannel == identifyCh2.CHANNEL) {
...
//      clamp.on();
      ...
      identifyCh1.setInput();
      identifyCh2.setInput();

      // Start 1st channel ID conversion
      identifyCh1.startConversion(AdcInterrupt_Enabled);
