...
public:
   /** Page of memory */
   uint8_t data[FlashImage::PAGE_SIZE];

   /** Indicates valid bytes in data */
   uint8_t validBits[FlashImage::PAGE_SIZE/8];
...
protected:
   FlashImage  &memoryImage;    //! Associated flash image
   uint32_t        address;        //! Current location in image

public:
   EnumeratorImp(FlashImage &memoryImage, uint32_t address = 0);
 ...
 */
EnumeratorImp::EnumeratorImp(FlashImage &memoryImage, uint32_t address) : memoryImage(memoryImage), address(address) {
   LOGGING_Q;
...
bool EnumeratorImp::nextValid() {
   ...
   address++;
   do {
      FlashImage::addressToPageOffset(address, pageNum, offset);
      //      log.print("Checking 0x%06X)\n", address);
      // At start of page or haven't checked this page yet
      if ((offset == 0) || (memoryPage == NULL)) {
         typename std::map<uint32_t, MemoryPagePtr>::iterator iter = memoryImage.memoryPages.find(pageNum);
      ...
      if (memoryPage == NULL) {
         ...
         address += FlashImage::PAGE_SIZE;
         address &= ~FlashImage::PAGE_MASK;
...
void EnumeratorImp::lastValid() {
   ...
   do {
      ...
      if ((offset == FlashImage::PAGE_SIZE-1) || !memoryPage->isValid(++offset)) {
         //         log.print("end=0x%06X\n", address);
         return;
      }
      address++;
 ...
 */
void FlashImage::setTargetType(TargetType_t targetType) {
   LOGGING;
 ...
 */
const char *FlashImage::getErrorString(USBDM_ErrorCode rc) {
   return UsbdmSystem::getErrorString(rc);
}

/**
 *  Initialises the memory to empty
 */
void FlashImage::clear(void) {
 ...
 */
bool FlashImage::isValid(uint32_t address) {
   uint16_t pageNum;
   uint16_t offset;
 ...
 */
FlashImage::EnumeratorPtr FlashImage::getEnumerator(uint32_t address) {
 ...
 */
void FlashImage::printMemoryMap() {
 ...
 */
unsigned FlashImage::getByteCount() const {
 ...
 */
const string & FlashImage::getSourcePathname() const {
   return sourcePath;
}
 ...
 */
MemoryPagePtr FlashImage::getmemoryPage(uint32_t pageNum) {
   MemoryPagePtr memoryPage;
 ...
 *  @param pageNum
 */
MemoryPagePtr FlashImage::allocatePage(uint32_t pageNum) {
   LOGGING_Q;
   MemoryPagePtr memoryPage;
 ...
 *  @return Error code
 */
USBDM_ErrorCode  FlashImage::loadFile(const string &filePath, bool clearBuffer) {
   LOGGING_Q;
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::saveFile(const std::string &filePath, bool discardFF) {
   LOGGING;
 ...
 */
void FlashImage::writeSrec(uint8_t *buffer, uint32_t address, unsigned size) {
   LOGGING_Q;
 ...
 */
void FlashImage::writeData(uint8_t *buffer, uint32_t address, unsigned size) {
   LOGGING;
   log.print("[0x%06X..0x%06X]\n", address, address+size-1);
   while (size>0) {
 ...
 */
uint8_t FlashImage::getValue(uint32_t address) {
   uint16_t         offset;
   uint16_t         pageNum;
   MemoryPagePtr    memoryPage;
   addressToPageOffset(address, pageNum, offset);
 ...
 */
void FlashImage::setValue(uint32_t address, uint8_t value) {
   uint16_t offset;
   uint16_t pageNum;
 ...
 *  @param address - Memory address
 */
void FlashImage::remove(uint32_t address) {
   uint16_t offset;
   uint16_t pageNum;
 ...
 *
 */
void FlashImage::dumpRange(uint32_t startAddress, uint32_t endAddress) {
   uint32_t addr;
   uint32_t rangeEnd;
 ...
 */
USBDM_ErrorCode FlashImage::loadData(uint32_t       bufferSize,
      uint32_t       address,
      const uint8_t data[],
      bool           dontOverwrite) {
 ...
 */
USBDM_ErrorCode FlashImage::loadDataBytes(uint32_t        bufferSize,
      uint32_t        address,
      const uint8_t   data[],
      bool            dontOverwrite) {
   LOGGING_Q;
...
void FlashImage::fill(uint32_t size, uint32_t address, uint8_t fillValue) {
   while (size-->0) {
      setValue(address++, fillValue);
   }
}

void FlashImage::fillUnused(uint32_t size, uint32_t address, uint8_t fillValue) {
   while (size-->0) {
      if (isValid(address)) {
 ...
 */
void FlashImage::addressToPageOffset(uint32_t address, uint16_t &pageNum, uint16_t &offset) {
 ...
 *  @note - These values do NOT refer to the paging structure used by the target!
 */
uint32_t FlashImage::pageOffsetToAddress(uint16_t pageNum, uint16_t offset) {
   if (offset>=PAGE_SIZE)
...
uint32_t FlashImage::targetToNative(uint32_t &value) {
   if (littleEndian) {
      return value;
   }
...
uint16_t FlashImage::targetToNative(uint16_t &value) {
   if (littleEndian) {
      return value;
   }
 ...
 *  @return Converted value
 */
int32_t FlashImage::targetToNative(int32_t &value) {
   if (littleEndian) {
      return value;
   }
 ...
 *  @return Converted value
 */
int16_t FlashImage::targetToNative(int16_t &value) {
   if (littleEndian) {
      return value;
   }
   return ((value<<8)&0xFF00) + ((value>>8)&0x00FF);
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::loadAbsoluteFile(const string &fileName) {
   LOGGING_Q;
   char         buffer[1024];
   uint32_t     addr = 0;
   uint32_t     size;
   ...
   log.print("FlashImage::MemorySpace::loadS1S9File()\n");
   printMemoryMap();
   return SFILE_RC_OK;
}
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::loadS1S9File(const string &fileName) {
   LOGGING_Q;
   char        *ptr;
   char         buffer[1024];
   uint32_t     addr;
   ...
   if (fp == NULL) {
      log.print("FlashImage::MemorySpace::loadS1S9File(\"%s\") - Failed to open input file\n", fileName.c_str());
      return SFILE_RC_FILE_OPEN_FAILED;
   }
   log.print("filename = \"%s\"\n", fileName.c_str());
 ...
 */
USBDM_ErrorCode FlashImage::checkTargetType(Elf32_Half e_machine, TargetType_t targetType) {
 ...
 */
USBDM_ErrorCode FlashImage::loadElfBlock(
      FILE       *fp,
      long        fOffset,
      Elf32_Word  size,
      Elf32_Addr  addr) {
 ...
 */
void FlashImage::printElfHeader(Elf32_Ehdr *elfHeader) {
 ...
 */
void FlashImage::fixElfHeaderSex(Elf32_Ehdr *elfHeader) {
 ...
 */
void FlashImage::printElfProgramHeader(Elf32_Phdr *programHeader, uint32_t loadAddress) {
   if (printHeader) {
 ...
 *  @param programHeader -  ELF Program header to print
 */
void FlashImage::printElfSectionHeader(Elf32_Shdr *elfsHeader) {
 ...
 */
void FlashImage::fixElfProgramHeaderSex(Elf32_Phdr *elfHeader) {
 ...
 *  @param elfHeader -  ELF Program header to convert
 */
void FlashImage::fixElfSectionHeaderSex(Elf32_Shdr *elfsHeader) {
 ...
 */
const char * FlashImage::getElfString(unsigned index) {
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::loadElfFile(const string &filePath) {
   LOGGING_Q;
   ...
   //   log.print("FlashImage::MemorySpace::loadElfFile() - \n");
   ...
   //   log.print("FlashImage::MemorySpace::loadElfFile() - elfHeader.e_machine = 0x%X(%d) \n", elfHeader.e_machine, elfHeader.e_machine);
   //   printElfHeader(&elfHeader);
 ...
 *
 * @param sectionHeader Section header to use
 *
 * @return  Associated header or nullptr if not found
 */
Elf32_Phdr *FlashImage::findRelatedProgramHeader(Elf32_Shdr *sectionHeader) {
   for(Elf32_Phdr *programHeader=programHeaders;
         programHeader<(programHeaders+elfHeader.e_phnum);
         programHeader++) {
      if ((sectionHeader->sh_offset>=programHeader->p_offset) &&
            ((sectionHeader->sh_offset+sectionHeader->sh_size)<=(programHeader->p_offset+programHeader->p_filesz))) {
 ...
 */
Elf32_Addr FlashImage::getLoadAddress(Elf32_Shdr *sectionHeader) {
   Elf32_Phdr *programHeader = findRelatedProgramHeader(sectionHeader);
   if (programHeader == nullptr) {
      throw new MyException("Elf file fomat error");
   }
   Elf32_Addr lAddress = sectionHeader->sh_addr;
   Elf32_Addr pAddress = sectionHeader->sh_addr-(programHeader->p_vaddr-programHeader->p_paddr);
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::loadElfBlockBySectionHeader(Elf32_Shdr *sectionHeader) {
   LOGGING_Q;
 ...
 *  @return Error code
 */
USBDM_ErrorCode FlashImage::loadElfBlockByProgramHeader(Elf32_Phdr *programHeader) {
   LOGGING_Q;
